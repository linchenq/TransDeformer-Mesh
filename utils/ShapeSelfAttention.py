...
class ShapeSelfAttentionLayer(nn.Module):
    ...
    def forward(self, x, xp, mask=None):
        # x: (B, N, E)
        # xp: (B, N, 2),  use template, not real position
        # m: mask in Attention
        x1, w = self.MHA(x, xp, x, xp, mask)
        x2 = x1 + x
        # add & norm
        x3 = x2 + self.MLP(x2)
        return x3, w
...
if __name__ == '__main__':
    ...
    model = ShapeSelfAttentionLayer(n_heads=16, 
                                    embed_dim=512,
                                    pos_dim=2, 
                                    alpha=100,
                                    attn_drop=0)
    ...
    x = torch.rand(1, N, 512)
    xp = torch.rand(1, N, 2)
    x = x.to(device)
    xp = xp.to(device)
    model = model.to(device)
    out, attn_weight = model(x, xp)
    
    free_gpu_mem, total_gpu_mem = torch.cuda.mem_get_info(device)
    print("attn output shape: {}".format(out.shape))
    print("attn weight shape: {}".format(attn_weight.shape))
